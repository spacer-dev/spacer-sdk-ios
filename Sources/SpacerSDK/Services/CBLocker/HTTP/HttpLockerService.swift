...
public class HttpLockerService {
    ...
    ///   - success: Callback on success
    ///   - failure: Callback on failure
    public func take(token: String, spacerId: String, lat:Double, Ing:Double, success: @escaping () -> Void, failure: @escaping (SPRError) -> Void) {
        let reqData = HttpLockerReqData(spacerId: spacerId)
    ...
    ///   - success: Callback on success
    ///   - failure: Callback on failure
    public func openForMaintenance(token: String, spacerId: String, lat:Double, Ing:Double, success: @escaping () -> Void, failure: @escaping (SPRError) -> Void) {
        let reqData = HttpLockerReqData(spacerId: spacerId)
