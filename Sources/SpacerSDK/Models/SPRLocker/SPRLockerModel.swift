...
public struct SPRLockerModel: Identifiable {
    ...
    public var description: String {
        return "id:\(id),status:\(status),size:\(size),closedWait:\(closedWait),version:\(version),doorStatus:\(doorStatus)"
    }
}
...
extension SPRLockerResData {
    func toModel() -> SPRLockerModel {
        ...
        return SPRLockerModel(id: id, status: status, size: size ?? "unknown", closedWait: closedWait, version: version, doorStatus: doorStatus)
    }
}
